...
const OfaPage = () => {
  ...
  const filteredRepairDevices = deviceData.filter(device => device.status === 'repair');
  ...
  const fetchDevicesStatusCount = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/dashboard-device/OFA');
  ...
  // Mengambil data produksi OFA 
  const fetchdeviceData = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/device/type/OFA');
        console.log("Data OFA:", response.data.data);
        if (response.data.success) {
        ...
        console.error("Gagal mengambil data OFA:", error);
        alert("Terjadi kesalahan saat mengambil data OFA.");
        router.push('/login'); 
    } finally {
        setLoading(false);
    }
    ...
    try {
        ...
        if (response.data.success) {
            ...
            setNewdeviceData({
                ...
                type: response.data.data.type || "OFA",
...
useEffect(() => {
  ...
  const fetchInstallationData = async () => {
    setLoading(true); 
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/installation/type/OFA');
...
useEffect(() => {
    ...
    axiosInstance.get('http://localhost:3000/api/installation/device/serial_number_device/OFA')
      .then(response => {
...
//UNTUK MENGAMBIL DATA SERIAL NUMBER UNTUK FORM UPDATE INSTALLATION
  const fetchSerialNumberData = async () => {
    try {
      const response = await axiosInstance.get('http://localhost:3000/api/installation/device/all-serial_number_device/OFA');
      if (response.data.success) {
...
}, [installationSearchQuery, installationStartDate, installationEndDate, installationData]);
...
const handleInstallationReset = () => {
  ...
  const fetchSerialNumberWithInstallation = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/maintenance/serial_number_with_installation/OFA');
...
useEffect(() => {
  ...
  const fetchMaintenanceData = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('http://localhost:3000/api/maintenance/maintenance/type/OFA');
      if (response.data.success) {
...
const handleAddMaintenance = async () => {
  ...
  const handleMaintenanceReset = () => {
    ...
    const fetchHistoryData = async () => {
      setLoading(true);
      try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/device-all-data/OFA');
  ...
  return (
    <div className="bg-gray-200 min-h-screen flex">
      <div className="flex-1 p-4 lg:p-0">
        <div className="p-4">
          <h2 className="text-xl font-semibold mb-4 -mt-2">Device OFA</h2>
