...
const sdfmsPage = () => {
  ...
  const fetchDevicesStatusCount = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/dashboard-device/SDFMS');
  ...
  // Mengambil data produksi SDFMS
  const fetchdeviceData = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/device/type/SDFMS');
        console.log("Data SDFMS:", response.data.data);
        if (response.data.success) {
        ...
        console.error("Gagal mengambil data SDFMS:", error);
        alert("Terjadi kesalahan saat mengambil data SDFMS.");
        router.push('/login'); 
    } finally {
        setLoading(false);
    }
...
// Menambah Data Produksi SDFMS
  const handleAddDevice = async (event) => {
    ...
    setserial_number_device((prevSerials) => [
      ...prevSerials,
      { serial_number_device: newDevice.serial_number_device, id_device: newDevice.id_device },
  ]);
  ...
  try {
      ...
      if (response.data.success) {
          ...
          setNewdeviceData({
              ...
              type: response.data.data.type || "SDFMS",
      ...
      setAlert({ message: "Terjadi kesalahan saat memperbarui data.", type: "error", visible: true });
...
// Delete Produksi SDFMS
  const handleDeleteDevice = async (id_device) => {
    if (!window.confirm(`Apakah Anda yakin ingin menghapus device dengan ID ${id_device}?`)) {
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setAlert({ message: `Device dengan ID ${id_device} berhasil dihapus.`, type: "success", visible: true });
        await fetchHistoryData();
        await fetchInstallationData();
        await fetchMaintenanceData();
        await fetchDevicesStatusCount();
      ...
      setAlert({ message: "Terjadi kesalahan saat menghapus device.", type: "error", visible: true });
    }
    setTimeout(() => {
      setAlert({ ...alert, visible: false });
    }, 2000);
  ...
  const fetchInstallationData = async () => {
    setLoading(true); 
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/installation/type/SDFMS');
  ...
  useEffect(() => {
    ...
    axiosInstance.get('http://localhost:3000/api/installation/device/serial_number_device/SDFMS')
      .then(response => {
...
//UNTUK MENGAMBIL DATA SERIAL NUMBER UNTUK FORM UPDATE INSTALLATION
  const fetchSerialNumberData = async () => {
    try {
      const response = await axiosInstance.get('http://localhost:3000/api/installation/device/all-serial_number_device/SDFMS');
      if (response.data.success) {
        setSerialNumberOptions(response.data.data); // Pastikan state untuk serial number options ada
      } else {
...
const openInstallationModal = (id_installation = null) => {
  if (id_installation) {
    ...
    if (serial_number_device.length === 0) {
     setAlert({ message: "Tidak ada serial number device yang tersedia. Silakan tambahkan device terlebih dahulu.", type: "warning", visible: true });  
      return;
    }
    setNewInstallationData({
      id_device: "",
...
if (deviceData.length > 0) {
  ...
  }, [installationSearchQuery, installationData]);
...
const handleInstallationReset = () => {
  ...
  const fetchSerialNumberWithInstallation = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/maintenance/serial_number_with_installation/SDFMS');
...
useEffect(() => {
  ...
  const fetchMaintenanceData = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('http://localhost:3000/api/maintenance/maintenance/type/SDFMS');
      if (response.data.success) {
 ...
 const handleMaintenanceDateChange = (e) => {
  ...
  const handleMaintenanceReset = () => {
    ...
    const fetchHistoryData = async () => {
      setLoading(true);
      try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/device-all-data/SDFMS');
        ...
        console.error("Gagal mengambil data Maintenance:", error);
        setAlert({ message: "Terjadi kesalahan saat mengambil data Maintenance.", type: "error", visible: true });
        router.push('/login'); 
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="bg-gray-200 min-h-screen flex">
      <div className="flex-1 p-4 lg:p-0">
        <div className="p-4">
          <h2 className="text-xl font-semibold mb-4 -mt-2">Device SDFMS</h2>
