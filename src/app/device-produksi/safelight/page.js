...
const SafeLightPage = () => {
  ...
  const fetchDevicesStatusCount = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/dashboard-device/SAFELIGHT');
  ...
  // Mengambil data produksi SAFELIGHT
  const fetchdeviceData = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/device/type/SAFELIGHT');
        console.log("Data SAFELIGHT:", response.data.data);
        if (response.data.success) {
        ...
        console.error("Gagal mengambil data SAFELIGHT:", error);
        alert("Terjadi kesalahan saat mengambil data SAFELIGHT.");
        router.push('/login'); 
    } finally {
        setLoading(false);
    }
...
// Menambahkan Data Produksi Safelight
...
// Update Data Produksi Safelight
  ...
  try {
      ...
      if (response.data.success) {
          ...
          setNewdeviceData({
              ...
              type: response.data.data.type || "SAFELIGHT",
...
useEffect(() => {
  filterData();
   ...
   const fetchInstallationData = async () => {
    setLoading(true); 
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/installation/type/SAFELIGHT');
  ...
  useEffect(() => {
    ...
    axiosInstance.get('http://localhost:3000/api/installation/device/serial_number_device/SAFELIGHT')
      .then(response => {
  ...
  }, []); // Menjalankan fetchData saat komponen dimuat
...
//UNTUK MENGAMBIL DATA SERIAL NUMBER UNTUK FORM UPDATE INSTALLATION
  const fetchSerialNumberData = async () => {
    try {
      const response = await axiosInstance.get('http://localhost:3000/api/installation/device/all-serial_number_device/SAFELIGHT');
      if (response.data.success) {
...
useEffect(() => {
  ...
  const fetchSerialNumberWithInstallation = async () => {
    setLoading(true);
    try {
        const response = await axiosInstance.get('http://localhost:3000/api/maintenance/serial_number_with_installation/SAFELIGHT');
...
useEffect(() => {
  ...
  const fetchMaintenanceData = async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get('http://localhost:3000/api/maintenance/maintenance/type/SAFELIGHT');
      if (response.data.success) {
...
const handleAddMaintenance = async () => {
 ...
 // Mengambil Data History (ALLDataDevice)
    const fetchHistoryData = async () => {
      setLoading(true);
      try {
        const response = await axiosInstance.get('http://localhost:3000/api/device/device-all-data/SAFELIGHT');
  ...
  return (
    <div className="bg-gray-200 min-h-screen flex">
      <div className="flex-1 p-4 lg:p-0">
        <div className="p-4">
          <h2 className="text-xl font-semibold mb-4 -mt-2">Device SAFELIGHT</h2>
